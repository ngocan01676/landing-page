...
class NewsController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        //$dataNews->whereDate('due_date', '>=', $now);
        if ($post_name) {
    ...
    public function commentNew(Request $request) {
        $dataNewsComment = NewsComment::query()->orderBy('id', 'desc');
        $dataNewsComment->where("blog_id","=",$request->id);
        $data = [
            'rows'        => $dataNewsComment->with("getAuthor")->paginate(20),
            'categories'  => NewsCategory::get(),
            'breadcrumbs' => [
                [
                    'name' => __('News comment'),
                    'url'  => 'admin/module/news/comment'
                ],
                [
                    'name'  => __('All'),
                    'class' => 'active'
            ...
            "languages"=>Language::getActive(false),
            "locale"=>\App::getLocale(),
            'page_title'=>__("New comment Management")
        ];
        return view('News::admin.news.comment', $data);
    } 
    ...
    public function create(Request $request)
    {
        ...
        $data = [
            ...
            'locations'        => NewsLocation::get()->toTree(),
            'row'         => $row,
            'breadcrumbs' => [
                [
                    'name' => __('News'),
    ...
    {   
        ...
        if ($action == "delete") {
            ...
            foreach ($ids as $id) {
                $query = News::where("id", $id);
                $this->hasPermission('news_manage_others');
