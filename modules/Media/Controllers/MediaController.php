...
class MediaController extends Controller
{
    ...
    {   
        ...
        if ($check) {
            try {
                ...
                $fullPath = str_replace('storage/app/private/','',$check);
                $path_r = 'private/'.\request()->get('path');
                $pathRoot = Storage::disk('local')->get($path_r);
    ...
    public function publicFileStore(Request $request)
    {   
        $user_id = "apec-hr";
        $fileName = 'file';
        $file = $request->file($fileName);
        ...
        if ($check) {
            try {
                $path = str_replace('public/','',$check);
                $path_a = str_replace('','',$check);
                $fullPath = str_replace('storage/app/public/','',$check);
                $path_r = 'public/'.\request()->get('path');
                $pathRoot = Storage::disk('local')->get($path_r);
                return response()->json(['data' => [
                    ...
                    'file_type'=>$file->getMimeType(),
                    'file_extension'=> $file->getClientOriginalExtension(),
                    'download'=>route('media.public.view',['path'=>$path]),
                    "url"=> Storage::url($check),
                    "full_path"=> $fullPath,
                    "pathRoot"=> $pathRoot
                ]]);
    ...
    public function publicFileView($id) {
        $contact = ConTact::find($id);
        $path = 'public/'.\request()->get('path'); 
        $test = Storage::disk('public')->path(str_replace("public/","",$contact->file_url));
        $content = file_get_contents($test);
        return response($content)->withHeaders([
            'content-type' => mime_content_type($test)
        ]);
        abort(404);
    }
}
